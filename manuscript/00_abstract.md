...
under-service do not co-occur in the same zone (partly structural: concentration
is tested only in hospital-dense zones), and 44 of 598 zones change diagnosis
between the two surfaces. A WorldPop control (modeled but residential) shows the
reversal tracks day-versus-night timing, not raster construction, and the
findings are stable to the hospital-count threshold, the coverage buffer, a
939-window CBSA re-run, and neutralizing the population LandScan places at
hospital sites.
